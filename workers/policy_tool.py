...
def _extract_access_level(task: str) -> Optional[int]:
    m = re.search(r"\blevel\s*([123])\b", (task or "").lower())
    if m:
    ...
    try:
        ...
        if needs_tool and any(kw in task.lower() for kw in ["ticket", "p1", "jira"]):
            ...
            if mcp_result.get("output"):
                policy_result["ticket_info"] = mcp_result["output"]

        # Step 4: Access policy details via MCP for Level-based requests.
        level = _extract_access_level(task)
        task_lower = task.lower()
        is_access_task = any(kw in task_lower for kw in ["access", "cấp quyền", "permission", "level"])
        if needs_tool and is_access_task and level is not None:
            access_call = _call_mcp_tool(
                "check_access_permission",
                {
                    "access_level": level,
                    "requester_role": "contractor" if "contractor" in task_lower else "employee",
                    "is_emergency": ("emergency" in task_lower or "khẩn cấp" in task_lower or "p1" in task_lower),
                },
            )
